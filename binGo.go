...
/*   Updated: 2025/04/25 11:26:36 by josfelip         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
func main() {
	...
	if len(participantes) < numGanhadores {
		fmt.Printf("Erro: numero de ganhadores (%d) maior que o numero de participantes (%d)\n", numGanhadores, len(participantes))
		os.Exit(1)
	}

	randomSource := rand.NewSource(time.Now().UnixNano())
	randomGenerator := rand.New(randomSource)
	randomGenerator.Shuffle(len(participantes), func(i, j int) {
